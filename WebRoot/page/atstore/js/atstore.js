...
$(function() {
	...
	function initData() {
		...
		$.ajax({
			type : "post",
			url : templateRoot + 'atstore/Atstore_pagelist.action',
			data : {
				"currPage" : currPage
			},
			dataType : "json",
...
//		tableHtml += "<td style='text-align: center; width:auto;'>ID</td>";
		tableHtml += "<td style='text-align: center; width:auto;'>商品名称</td>";
		tableHtml += "<td style='text-align: center; width:auto;'>数量</td>";
		tableHtml += "<td style='text-align: center; width:auto;'>单位</td>";
...
//			tableHtml += "<td style='text-align: center;'>" + item.id + "</td>";
	...
	$("#deleteB").click(function() {
		...
		hConfirm('您确定要删除所选记录吗？', null, function(r) {
			...
			$.ajax({
				type : "post",
				url : templateRoot + 'atstore/Atstore_deleteByIds.action',
				data : {
					"ids" : idsArr
				},
				dataType : "json",
	...
	$("#testB").click(function() {
		$.ajax({
			type : "post",
			url : templateRoot + 'atstore/Atstore_atstoreTest.action',
			dataType : "json",
			cache : false,
			success : function(data, textStatus) {
				hAlert(data.message);
	...
	function updateDialog(id) {
		...
		$.ajax({
			type : "post",
			url : templateRoot + 'atstore/Atstore_getById.action',
			data : {
				"relationId" : id
			},
			dataType : "json",
	...
	function save() {
		...
		$.ajax({
			type : "post",
			url : templateRoot + 'atstore/Atstore_save.action',
			data : {
